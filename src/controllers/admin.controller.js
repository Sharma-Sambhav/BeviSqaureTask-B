...
// Unblock/Reactivate a rejected user
const unblockUser = asyncHandler(async (req, res) => {
    try {
        ...
        if (user.status !== "rejected") {
            throw new APIError(400, "User is not in rejected status. Only rejected users can be unblocked.");
        }

        // Set user back to pending status so they can reapply or be re-approved
        user.status = "pending";
        await user.save();
        ...
        return res
            ...
            .json(
                new APIresponse(
                    200,
                    { user: updatedUser },
                    "User unblocked successfully. User status changed to pending."
                )
            );
    } catch (error) {
        console.log('File: admin.controller.js', 'Line 145:', error);
        throw new APIError(error.statusCode || 500, error?.message || "Something went wrong while unblocking user");
    }
});
